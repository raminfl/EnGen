...
class GenerateEnGen(object):
    def __init__(self, model_path, args_path, csv_path, source_patient_ids, source, target, iter_id, p, current_file_path, random_state=42):

        
        self.random_state = random_state
        self.source_patient_ids = source_patient_ids
    ...
    def generate_csv(self):
        # generate the .csv sample for a train patient

        for source_id in self.source_patient_ids:    
            print('generating for train patient {}'.format(source_id))
            df_source = pd.read_csv(os.path.join(self.current_file_path,'../data/preprocessed/Pre/Func_Pheno_20k_{0:}_A_Pre.csv'.format(source_id)))
            ...
            df_gen_target.to_csv(self.csv_path+'generated_{}_scaled.csv'.format(source_id), header=True, index=False)
            df_gen_target.iloc[:,:] = scaler.inverse_transform(df_gen_target.iloc[:,:].values)
            df_gen_target.to_csv(self.csv_path+'generated_{}.csv'.format(source_id), header=True, index=False)
...
if __name__ == "__main__":
    ...
    for iter_id in range(n_iterations):
        ...
        path = '../EnGen_train_iterations/engen_output/'        
        ...
        train_ids = p['AE_train_ids']
        model_path = path+iter_folder+'/saved_model/best_model_engen.pth'
        args_path = path+iter_folder+'/commandline_args.txt'
        csv_path = path+iter_folder+'/generated_train/'
        
        
        generate_engen = GenerateEnGen(model_path, args_path, csv_path, train_ids, source, target, iter_id, p, current_file_path)
