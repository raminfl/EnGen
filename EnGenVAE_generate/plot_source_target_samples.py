...
def extract_feature(current_file_path, n_clusters=25):
    ...
    df_gt_st = df_gt_st.sample(frac=1, random_state=42)
    ...
    for source_patient_id in all_patient_ids:
        ...
        frequencies_row_Pre.append(nan) # model iter_id
        frequencies_row_Pre.append(nan) # generated sample i
        ...
        frequencies_row_1hr.append(nan) # model iter_id
        frequencies_row_1hr.append(nan) # generated sample i
    ...
    for iter_id in range(n_iterations):
        ...
        for i in range(27):
            ...
            gen_path = os.path.join(current_file_path,'../EnGenVAE_train_iterations/engen_vae_output/{}/generated/'.format(iter_folder))
            data_type = 'gen'
            df_sample_gen = pd.read_csv(gen_path+'generated_{0:}_{1:}.csv'.format(iter_folder,i))
            ...
            frequencies_row_gen.append(nan) # patient_id
            frequencies_row_gen.append(iter_id) # model iter_id
            frequencies_row_gen.append(i) # generated sample i
        ...
        df_all_features = pd.DataFrame(data=all_frequencies_iter, columns=['patient_id','iter_id','i','timepoint','data_type']+['c_{}'.format(i) for i in range(n_clusters)])

        df_all_features.to_csv(os.path.join(current_file_path,'../EnGenVAE_train_iterations/engen_vae_output/{0:}/Features_gt_Pre_gt_1hr_gen_1hr_test_train_kmeans{1:}_{0:}.csv'.format(iter_folder, n_clusters)), header=True, index=False)
   



def PCA_all_samples(current_file_path, n_clusters=25):
    random.seed(42)
    ...
    for iter_id in range(n_iterations):
        if iter_id < 10:
            ...
            iter_folder = 'iter_{}'.format(iter_id)
        p = pickle.load(open(os.path.join(current_file_path,'../EnGen_train_iterations/training_data/{}/scaler_kmeans_25.pkl'.format(iter_folder)), 'rb'))
        df = pd.read_csv(os.path.join(current_file_path,'../EnGenVAE_train_iterations/engen_vae_output/{0:}/Features_gt_Pre_gt_1hr_gen_1hr_test_train_kmeans{1:}_{0:}.csv'.format(iter_folder, n_clusters)))
        
        df_gt = df[df['data_type']=='ground_truth']
        df_gt.reset_index(inplace=True, drop=True)
        scaler = StandardScaler()
        #print('fitting scaler')
        scaler.fit(df_gt.iloc[:,5:].values)
        df_gt.iloc[:,5:] = scaler.transform(df_gt.iloc[:,5:].values)
        df.iloc[:,5:] = scaler.transform(df.iloc[:,5:].values)
        if iter_folder == 'iter_00':
            pca = PCA(n_components=2)
            pca.fit(df_gt.iloc[:,5:])
            p['pca_Pre_1hr_gt'] = pca
            pickle.dump(p, open(os.path.join(current_file_path,'../EnGenVAE_train_iterations/engen_vae_output/pca_gt_{}_clusters.pkl'.format(n_clusters)), 'wb'))
        else:
            pca = pickle.load(open(os.path.join(current_file_path,'../EnGenVAE_train_iterations/engen_vae_output/pca_gt_{}_clusters.pkl'.format(n_clusters)), 'rb'))['pca_Pre_1hr_gt']

        ev0, ev1 = pca.explained_variance_ratio_
        df_pca = pd.DataFrame()
        df_pca['patient_id'] = df['patient_id']
        df_pca['iter_id'] = df['iter_id']
        df_pca['i'] = df['i']
        ...
        df_pca.iloc[:,5:] = pca.transform(df.iloc[:,5:])
        # id_list = list(df_pca['patient_id'].values)
        # import math
        # id_list = [x for x in id_list if math.isnan(x) == False]
        # patient_ids = sorted(list(set(id_list)))
        for idx, row in df_pca.iterrows():
            # print(row)
            style = {}
            style['s'] = 20
            if row['data_type'] == 'ground_truth' and iter_folder == 'iter_00':
            ...
            elif row['data_type'] == 'gen':
                style['marker']= 'x'
                if row['timepoint'] == 'Pre':
                    # style['c']= 'black' # not applicable
                    return
                else:
                    # style['c']= (random.randint(0, 255), random.randint(0, 255), random.randint(0, 255)) # '#e17055'
                    random.seed(42+iter_id)
                    style['c'] = "#"+''.join([random.choice('0123456789ABCDEF') for j in range(6)])
                ax.scatter(row['PC1'], row['PC2'],**style)
    ...
    legend_elements = [mlines.Line2D([], [], color='#6FC2B3', marker='s', linestyle='None', markerfacecolor='None', markersize=5, label='GT Pre'),
                        ...
                        mlines.Line2D([], [], color='#e17055', marker='x', linestyle='None', markerfacecolor='#e17055', markersize=5, label='Gen')]

    ax.legend(handles=legend_elements, loc='best')
    # plt.show()
    
    plt.savefig(os.path.join(current_file_path,'../EnGenVAE_train_iterations/engen_vae_output/PCA_source_target.pdf'), format='pdf', dpi=600)
...
if __name__ == "__main__":
    ...
    # perform clustering and extract features for samples 
    extract_feature(current_file_path)

    PCA_all_samples(current_file_path)
